...
func RGBAvg(img *image.RGBA, x, y int, canalHalfSide int, differenceMax uint8) [imagelib.NumColorsRGB]uint8 {

	if img == nil {
		return [imagelib.NumColorsRGB]uint8{}
	}

	offset := (y-img.Rect.Min.Y)*img.Stride + (x-img.Rect.Min.X)*imagelib.NumColorsRGBA
	...
	var cnt int
	var sum [imagelib.NumColorsRGB]int

	xWidth, yHeight := rect.Max.X-rect.Min.X, rect.Max.Y-rect.Min.Y
	offsetY := (rect.Min.Y-img.Rect.Min.Y)*img.Stride + (rect.Min.X-img.Rect.Min.X-1)*imagelib.NumColorsRGBA
	...
	X:
		for x1 := 0; x1 < xWidth; x1++ {
			...
			for i := 0; i < imagelib.NumColorsRGB; i++ {
	...
	if cnt <= 0 {
		return [imagelib.NumColorsRGB]uint8{clr[0], clr[1], clr[2]}
	}

	return [imagelib.NumColorsRGB]uint8{uint8(sum[0] / cnt), uint8(sum[1] / cnt), uint8(sum[2] / cnt)}
}
